...
def export_universe(df: pd.DataFrame, before_format: Optional[str] = None, after_format: Optional[str] = None):
    # Export a list of ticker symbols to Excel with auto column width.
    if before_format is None:
        before_format = os.path.abspath("data_clean/universe.xlsx")
    if after_format is None:
        after_format = os.path.abspath("data_clean/universe_formatted.xlsx")

    # Ensure the folder exists
    os.makedirs(os.path.dirname(before_format), exist_ok=True)
    os.makedirs(os.path.dirname(after_format), exist_ok=True)
    ...
    os.startfile(os.path.abspath(after_format))
